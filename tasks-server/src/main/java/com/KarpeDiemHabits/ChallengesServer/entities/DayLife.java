...
public class DayLife {
    ...
    //@JsonIgnore
    private List<Challenge> approvedTasks = new ArrayList<>();

    @ManyToMany
    @JoinColumn(name = "challenge_id")
    //@JsonIgnore
    private List<Challenge> tasks = new ArrayList<>();

   

    public boolean deleteChallenge( Challenge challenge ){
        return getTasks().remove(challenge) &&
        getApprovedTasks().remove(challenge)&&
